...
async function getClient() {
  ...
  if (txStack.length) return txStack[txStack.length - 1];
  return pool.connect();
}

async function releaseClient(client) {
  if (!usingPg) return;
  if (txStack.length) return; // don't release transaction client here
...
export default {
  ...
  transaction(fn) {
    if (usingPg) {
      ...
      // For sqlite, fn is synchronous normally, but allow async too
      try {
        const t = sqliteDb.transaction(fn);
        return t();
      } catch (err) {
        // If sqlite transaction fails, throw
        throw err;
      }
    }
  },

  // helper exec for running multi-statement SQL (only for sqlite mode or pg simple)
  async exec(sql) {
    if (usingPg) {
      const client = await pool.connect();
      try {
  ...
  // utility log functions matching original exports
  logAudit(userId, action, entity, entityId = null, details = null, ip = null) {
    const detailsText = details ? JSON.stringify(details) : null;
    if (usingPg) {
