...
export default function AdminUserActivityPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => adminApi.getUserActivity(userId, { page, limit: 30 }),
  ...
  return (
    <PageContainer maxWidth="lg">
      <PageHeader title="Activity" description="Recent actions" />

      {isLoading ? (
        <div className="space-y-3">{[...Array(8)].map((_, i) => <Skeleton key={i} className="h-16" />)}</div>
      ) : !activities.length ? (
        <EmptyState icon={<Activity size={28} />} title="No activity yet" description="No actions recorded for this user" />
      ) : (
        <>
          <div className="relative">
            ...
            <div className="space-y-1">
              {activities.map((entry, i) => {
                const entityType = (entry as unknown as { entity_type?: string }).entity_type ?? 'application';
                ...
                return (
                  <div key={entry.id} className="flex gap-4 relative">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 z-10 border-2 border-[var(--bg)] ${color}`}>{icon}</div>
                    <div className="flex-1 pb-4 pt-2">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="text-[10px] text-[var(--text-secondary)] shrink-0 mt-0.5">{timeAgo(entry.created_at)}</span>
                      </div>
                    </div>
                  </div>
                );
          ...
          {pagination && pagination.total_pages > 1 && (
            <div className="mt-6"><Pagination page={page} totalPages={pagination.total_pages} onPageChange={setPage} /></div>
          )}
        </>
      )}
    </PageContainer>
